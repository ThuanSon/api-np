      ...
      arrResult.map(async (item, index) => {
        ...
        if (results.length === 0) {
          ...
          if (response?.length > 0) {
             ...
             */
            console.log(item);
            if (response[2]?.fl === "noun") {
              wordStringTemp = item;
              wordString = response[1]?.hwi?.hw;
            }
            const plural = `select * from words where name = '${wordString}'`;
            const [resultPlural] = await db.promise().query(plural);
            if (resultPlural.length === 0) {
            ...
            resArr[index] = {
              id: null,
              name: wordStringTemp ? wordStringTemp : wordString,
              type: wordType,
              expandType: null,
              kind: kind,
              position: position,
          ...
          // const plural = results.find((item) => item.name === );
          if (possessiveS) {
